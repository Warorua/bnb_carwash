                                            ...
                                            @if (!empty($subcategory))
                                                <optgroup label="{{ $checkoout->checkout_point }}">
                                                    @foreach ($subcategory as $subcategorys)
                                                      <option value="{{ $subcategorys->id }}" data-category="{{ $checkoout->checkout_point }}" data-point="{{ $subcategorys->checkout_point }}" data-service-charge="{{ $subcategorys->service_charge ?? 0 }}">
                                                ...
                                                <table class="table table-bordered table-sm main_data_points f-14" id="main_data_points" align="center">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th class="fw-bold" style="width:12%;">{{ trans('message.Select Product') }}
                                                            </th>
    ...
    <?php } else {
        foreach ($data as $datas) { ?>
            ...
            <tr class="obs_point_data" id="<?php echo 'row_id_delete_' . $i; ?>">
                ...
                <td>
                    <select name="product2[product_id][]" class="form-control product_ids product1s_{{ $i }} searchable-select" url="{{ url('/jobcard/getprice') }}" row_did="{{ $i }}" id="product1s_{{ $i }}" qtyappend="" required>
                        <option value="">
                ...
                <td>
                    <input type="text" name="product2[price][]" class="form-control prices rate product1_<?php echo $i; ?> price_<?php echo $i; ?>" id="product1_<?php echo $i; ?>" row_id="{{ $i }}" maxlength="8">
                </td>
                                        ...
                                        <table class="table table-bordered addtaxtype" id="tab_products_detail" align="center">
                                            ...
                                            <tbody>
                                                ...
                                                <tr id="<?php echo 'row_id_' . $id; ?>">
                                                    ...
                                                    <td>
                                                        <input type="text" name="other_price[]" class="form-control other_service_price othr_price_<?php echo $id; ?>" id="oth_price" value="<?php echo $product->total_price; ?>" maxlength="8">
                                                    </td>
                                                    <td class="text-center">
...
document.addEventListener('input', function(e) {
    if (e.target.matches('.other_service_price')) {
    ...
    $(document).ready(function () {
        ...
        // Initialize Select2 for dropdown
         $('#searchable-select').select2({
            ...
            closeOnSelect: false,
            allowClear: true,
            minimumInputLength: 0, // Show search immediately
            templateResult: formatCheckpointOption, // Optional: custom formatting
            templateSelection: formatCheckpointSelection // Optional: custom selection display
        }).on('select2:close', updateSelect2Placeholder);

        // Optional: Custom formatting functions
        function formatCheckpointOption(option) {
            if (!option.id) { return option.text; }
            return $('<span>' + option.text + '</span>');
        }

        function formatCheckpointSelection(option) {
            return option.text;
        }
        ...
        $('#searchable-select').on('change', function () {
            ...
            selectedOptions.forEach((option) => {
                ...
                const serviceCharge = option.element.dataset.serviceCharge || 0;
                // Prevent duplicates
                ...
                const newRow = `
                    <tr id="row-${categoryId}" class="obs_point_data">
                        ...
                        <td><input type="number" name="product2[service_charge][]" class="form-control charge" data-row-index="${rowIndex}" value="${serviceCharge}" maxlength="8"></td>
                        <td>
                        ...
                        <td>
                            <input type="number" name="product2[qty][]" class="form-control qtyt"  data-row-index="${rowIndex}" prd_url="<?php echo url('/sale_part/get_available_product'); ?>" value="1" />
                        </td>
                         <td>
...
<script nonce="{{ $cspNonce }}">
    $(document).ready(function() {
        ...
        $('body').on('change keyup', '.qtyt', function() {
            ...
            // Get product ID - works for both new rows (.product_ids) and existing rows (.product1s_*)
            var productid = $('.product1s_' + row_id).find(":selected").val();
            if (!productid) {
                // Try alternative selector for dynamically added rows
                productid = $this.closest('tr').find('.product_ids').val();
            }

            var qty = parseFloat($this.val()) || 0;
            var price = parseFloat($('.product1_' + row_id).val()) || 0;
            ...
            var msg5 = "{{ trans('Product Not Available.Please Add Product stock.!') }}";
            var msg6 = "{{ trans('message.Current Stock :') }}";
            var msg7 = "{{ trans('message.First select product name') }}";
            var msg8 = "{{ trans('message.OK') }}";
            var msg9 = "{{ trans('Quantity must be greater than 0') }}";
            ...
            // Validate: only numbers (no letters/special chars)
            ...
            // Calculate total quantity for same product across all rows
            var totalQtyForProduct = 0;
            $('.qtyt').each(function() {
                var $row = $(this).closest('tr');
                // Check both class patterns: .product_ids (new rows) and .product1s_* (existing rows)
                var rowProductId = $row.find('.product_ids').val() || $row.find('select[name="product2[product_id][]"]').val();
                if (rowProductId == productid) {
                    totalQtyForProduct += parseFloat($(this).val()) || 0;
                    console.log("Total Quantity :"+totalQtyForProduct);
                }
            });

            // Clear any existing timeout for this input
            clearTimeout($this.data('stockCheckTimeout'));

            // Set new timeout for stock check (debounce)
            var timeoutId = setTimeout(function() {
                $.ajax({
                    type: 'GET',
                    url: url,
                    data: {
                        qty: totalQtyForProduct,  // Send total quantity
                        productid: productid
                    },
                    success: function(response) {
                        if (response.success == '1') {
                            // Stock not available
                            swal({
                                title: msg5 + ' ' + msg6 + ' ' + response.currentStock,
                                cancelButtonColor: '#C1C1C1',
                                buttons: {
                                    cancel: msg8,
                                },
                            ...
                            // Reset the current row's quantity and total
                            if ($('.qty_' + row_id).length > 0) {
                                $('.qty_' + row_id).val('');
                                $('.total1_' + row_id).val('');
                            }

                            // For new rows (dynamically added - uses data-row-index)
                            var $currentRow = $this.closest('tr');
                            $currentRow.find('.qtyt').val('');
                            $currentRow.find('.total1').val('');
                        } else {
                            // Stock available - calculate total with discount and tax
                            ...
                            // Store qty attribute for later use
                            $('#product1s_' + row_id).attr('qtyappend', qty);
                        }
                    },
                    error: function(e) {
            ...
            $this.data('stockCheckTimeout', timeoutId);
        });
