                        ...
                        <div class="row mt-3 row-mb-0">
                            <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 {{ $errors->has('start_date') ? ' has-error' : '' }}">
                                ...
                                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8 date">
                                    <input type="text" name="start_date" id="start_date_sales" autocomplete="off" class="form-control start_date datepicker" value="{{ !empty($s_date) ? date(getDateFormat(), strtotime($s_date)) : date(getDateFormat(), strtotime('first day of this month')) }}" placeholder="<?php echo getDatepicker(); ?>" required />
                            ...
                            <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 {{ $errors->has('end_date') ? ' has-error' : '' }}">
                                ...
                                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8 date">
                                    ...
                                    <input type="text" name="end_date" id="end_date_sales" autocomplete="off" class="form-control end_date datepicker" value="{{ old('p_date', date('Y-m-d')) }}" placeholder="<?php echo getDatepicker(); ?>" required />
...
<script nonce="{{ $cspNonce }}">
  $(document).ready(function() {
    ...
    $(".colorname").click(function() {
        ...
        if (start_date === "") {
            $('#common_error_span').css({"display": ""});
        } else {
            $('#common_error_span').css({"display": "none"});
        }
        if (end_date === "") {
            $('#common_error_span_end').css({"display": ""});
            return false;
        } else {
            $('#common_error_span_end').css({"display": "none"});
        }
    });
    ...
    // **FIX: Only initialize chart if the container exists**
    var chartStatus = '<?php echo $chartStatus; ?>';
    if (chartStatus === 'yes' && $('#service_report').length > 0) {
    ...
    function drawChart() {
        // **FIX: Check if container exists before drawing**
        if ($('#service_report').length === 0) {
    ...
    // Make drawChart available globally for the "View Chart" button
    window.drawChart = drawChart;

    // Prevent keyboard input on date picker fields
    $('.datepicker, .date_simple_class, .productDate').on('keypress keydown', function(e) {
        e.preventDefault();
        return false;
    });

    $('body').on('keypress keydown', 'input[type="date"], .prevent-keyboard', function(e) {
        e.preventDefault();
        return false;
    });
});
...
function myFunction() {
    ...
    if (service === 'yes') {
        // **FIX: Check if container exists before showing**
        if ($('#service_report').length === 0) {
        ...
        if (x.style.visibility === "hidden") {
            ...
            // **FIX: Redraw chart when showing to ensure proper rendering**
