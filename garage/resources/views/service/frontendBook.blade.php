...
<head>
  ...
  {{-- E:\xampp\htdocs\garagemaster_web\vendors\bootstrap-date-time-picker\bootstrap5\css\bootstrap-datetimepicker.css --}}
   ...
   /* Add this CSS in your <style> section in the <head> */
                    ...
                    <div class="row">
                        <div class="col-sm-4 col-md-4 sidebar">
                            ...
                            <div class="w-auto mx-0 mt-5">
                                <button data-bs-toggle="modal" data-bs-target="#myModal" class="bookService rounded-right">{{ trans('message.Book an appoinment') }}</button>
                            </div> 
                            ...
                            <div class="row col-md-6 col-lg-6 col-xl-6 col-xxl-6 col-sm-6 col-xs-6 display-content">
                                ...
                                <div class="col-md-8 col-lg-8 col-xl-8 col-xxl-8 col-sm-8 col-xs-8 width-100 date">
                                    <!-- <input type='text' class="form-control datepicker" name="s_date" autocomplete="off" id='s_date' placeholder="<?php echo getDatepicker();
                                       ...
                                       <input type='text' class="form-control datepicker" name="s_date" autocomplete="off" id='s_date' 
                                        placeholder="<?php echo getDatepicker(); echo ' hh:mm:ss'; ?>" 
...
$(document).ready(function() {
    // Prevent keypress on datepicker inputs
...
if (!empty($holiday)) {
    foreach ($holiday as $holidays) {
        ...
        $service_data_array[] = [
            'title' => substr($holidays->title, 0, 10), 
...
// Convert to JSON - always return a valid JSON array
if (!empty($service_data_array)) {
    $data1 = json_encode($service_data_array);
} else {
    $data1 = '[]'; // Return empty array instead of '0'
}
?>
...
<script nonce="{{ $cspNonce }}">
       document.addEventListener('DOMContentLoaded', function() {
        ...
        // Parse events data - ensure it's an array
        var eventsData = [];
        try {
            var rawData = <?php echo !empty($data1) ? $data1 : '[]'; ?>;
            if (rawData && rawData !== '0' && Array.isArray(rawData)) {
                eventsData = rawData;
            }
        } catch(e) {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            // Static events array - NO URL fetching
            events: eventsData,
            
            // Explicitly disable event source URL fetching
            eventSources: [],
...
<script nonce="{{ $cspNonce }}">
$(document).ready(function() {
    console.log('Modal element exists:', $('#myModal').length > 0);
