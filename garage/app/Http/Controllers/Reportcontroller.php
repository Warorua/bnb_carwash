...
class Reportcontroller extends Controller
{
    ...
    public function record_service(Request $request)
    {
        ...
        if (isAdmin(Auth::User()->role_id)) {
            ...
            if ($s_date == '' && $e_date == '' && $all_service == 'all') {
                $service = DB::select('SELECT count(*) as counts, RIGHT(YEAR(`service_date`),4) as date FROM tbl_services where (done_status=1) and (soft_delete=0) group by YEAR(`service_date`) ORDER BY date ASC');
                $title_report = 'All Service';
                $date_report = 'Year';
                $title = 'Service';
                ...
                $service = DB::select("SELECT count(*) as counts, RIGHT(YEAR(`service_date`),4) as date FROM tbl_services where (service_type='$all_service') and (done_status=1) and (soft_delete=0) group by YEAR(`service_date`) ORDER BY date ASC");
                $title_report = 'All Service';
                $date_report = 'Year';
                $title = 'Service';

                $servicereport = Service::where([['done_status', '=', 1], ['service_type', '=', $all_service], ['soft_delete', 0]])->orderBy('id', 'desc')->get();
            } elseif ($s_date == '' && $e_date == '' && $all_service == 'paid') {
                ...
                if ($select_customerId != '') {
                    // FIXED: Corrected the date order from '$e_date' AND '$s_date' to '$s_date' AND '$e_date'
                    $service = DB::select("select count(*) as counts, CONCAT(MONTHNAME(`service_date`),'-',RIGHT(YEAR(`service_date`),4)) as date FROM tbl_services where (service_date BETWEEN '$s_date' AND '$e_date') and (done_status=1) and (soft_delete=0) and (customer_id='$select_customerId') GROUP BY MONTH(service_date), YEAR(service_date)");

                    $title_report = 'All Service';
                    $date_report = 'Year';
                    $title = 'Service';

                    $servicereport = Service::where([['done_status', '=', 1], ['customer_id', '=', $select_customerId], ['soft_delete', 0]])
                        ->whereBetween('service_date', [$s_date, $e_date])->orderBy('id', 'desc')->get();
                } else {
                    $service = DB::select("select count(*) as counts, CONCAT(MONTHNAME(`service_date`),'-',RIGHT(YEAR(`service_date`),4)) as date FROM tbl_services where (service_date BETWEEN '$s_date' AND '$e_date') and (done_status=1) and (soft_delete=0) GROUP BY MONTH(service_date), YEAR(service_date)");

                    $title_report = 'All Service';
                    $date_report = 'Year';
                    $title = 'Service';

                    $servicereport = Service::where([['done_status', '=', 1], ['soft_delete', 0]])
                        ->whereBetween('service_date', [$s_date, $e_date])->orderBy('id', 'desc')->get();
                }
            } elseif ($s_date != '' && $e_date != '' && $all_service == 'free') {
                if ($select_customerId != '') {
                    $service = DB::select("select count(*) as counts, CONCAT(MONTHNAME(`service_date`),'-',RIGHT(YEAR(`service_date`),4)) as date FROM tbl_services where (service_date BETWEEN '$s_date' AND '$e_date') and (done_status=1) and (soft_delete=0) and (service_type='$all_service') and (customer_id='$select_customerId') GROUP BY MONTH(service_date), YEAR(service_date)");
                    ...
                    $service = DB::select("select count(*) as counts, CONCAT(MONTHNAME(`service_date`),'-',RIGHT(YEAR(`service_date`),4)) as date FROM tbl_services where (service_date BETWEEN '$s_date' AND '$e_date') and (done_status=1) and (soft_delete=0) and (service_type='$all_service') GROUP BY MONTH(service_date), YEAR(service_date)");
