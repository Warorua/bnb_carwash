...
class IncomeController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        // **FIX: Calculate Outstanding Amount**
        $outstanding_amount = 0;
        if ($first_data) {
            // Get the invoice details
            $invoice = DB::table('tbl_invoices')
                ->where('invoice_number', $first_data->invoice_number)
                ->first();
            
            if ($invoice) {
                // Calculate outstanding amount = grand_total - paid_amount
                $outstanding_amount = $invoice->grand_total - $invoice->paid_amount;
            }
        }

        return view('income/edit', compact('first_data', 'sec_data', 'customer', 'invoice_no', 'tbl_payments', 'tbl_custom_fields', 'branchDatas', 'outstanding_amount'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // **FIX: Get Total_Amount from hidden input instead of disabled input**
        $Total_Amount = $request->Total_Amount_hidden; // Use hidden field value
        $income_entry = $request->income_entry;
        $amount_recevied1 = 0;
        ...
        // **FIX: Add validation message if amount exceeds outstanding**
        if ($Total_Amount < $amount1) {
            return redirect('income/edit/' . $id)
                ->with('message_error', 'Income amount (' . $amount1 . ') cannot exceed outstanding amount (' . $Total_Amount . ')');
        }
        ...
        if ($Total_Amount >= $amount1) {
            ...
            $tbl_payment_records->payment_type = $request->Payment_type;
            $tbl_payment_records->payment_date = $dates;
            $tbl_payment_records->branch_id = $request->branch; // **FIX: Use actual branch_id from request**
            $tbl_payment_records->save();

            return redirect('income/list')->with('message', 'Income Updated Successfully');
        } else {
            return redirect('income/edit/' . $id)->with('message_error', 'Income amount cannot exceed outstanding amount');
        }
    }
